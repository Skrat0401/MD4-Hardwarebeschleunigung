...
begin
    ...
    begin
        if rst = '1' then
            ...
            if (ram_addr < "10010" and rd_en_to_md4_buff = '1') then -- max 0xF
                if(ram_addr > "00000") then
               cache_array((to_integer(ram_addr))- 1) <= data_to_md4;    
                end if;
            ram_addr                          <= ram_addr + 1;
            end if;
            if (ram_addr = "10000") then
                array_full <= '1';
            end if;
            if state = ST then--done = '1' or busy = '1' then
                array_full <= '0';
            end if;
            if empty_to_md4 = '1' then
                ram_addr <= "00000";
            ...
            case state is
                ...
                when GIVE =>
                    ...
                    if (counter < 16) then
                        ...
                        if counter = 12 then
                            test_int1 <= cache_array(counter);
                        end if;
                        if counter = 13 then
                            test_int2 <= cache_array(counter);
                        end if;
                        if counter = 14 then
                            test_int3 <= cache_array(counter);
                        end if;
                        if counter = 15 then
                            test_int4 <= cache_array(counter);
                        end if;
                        counter <= counter + 1;
                    else
                        counter <= 0;
